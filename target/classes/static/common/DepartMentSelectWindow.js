...
DepartMentSelectWindow = function() {
	DepartMentSelectWindow.superclass.constructor.call(this);
	this.initControls();
	this.initEvents();
};
mini
		.extend(
				...
				mini.Window,
				{
					...
					initControls : function() {
						...
						var topHtml = '<div style="padding:5px;text-align:center;">'
								+ '<span id="'
								+ labelId
								+ '">'
								+ this.keyLable
						...
						//footer
						var footerHtml = '<div style="padding:8px;text-align:center;">'
					...
					setUrl : function(value) {
						this.url = value;
						this.tree.setUrl(value);//树组件通过此方法获取数据
					},
					...
					setData : function(data, callback) {
						...
						var nodes = this.tree.findNodes(function(node) {
							for (var i = 0; i < data.length; i++) {
								if (data[i].deptId == node.deptId) {
									return true;
								}
							}
						});
						this.tree.checkNodes(nodes);
					},
					getData : function() {
						var nodes = [];
...
mini.regClass(DepartMentSelectWindow, "DepartMentSelectWindow");